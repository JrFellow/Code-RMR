...
class Authentication(Content): # everything auth-based
    ...
    def update_user_credential(username, credential_index, new_value, filename):
        ...
        if match:
            ...
            print("mam", updated_line_parts[credential_index])
            updated_line_parts[credential_index] = f" {int(updated_line_parts_before_change) + new_value_int}" + "]"
            print("man", updated_line_parts[credential_index])
    ...
    class car:
        ...
        def move(self, speed, endpoint):
            ...
            if self.y > endpoint and moved == False:
                print("moved ")
                self.y -= speed
                print("self.y = ", self.y, endpoint)
            elif self.y == endpoint:
                moved = True
                print("Reached but i want to move more liam hiader mutiny coming")
                self.y = endpoint
    ...
    while run:  # loop while the code is running
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_RETURN and user_answer != "":
                    ...
                    if int(user_answer) == int(math_question[2]):
                        mph = 20
                        print(math_question[2])
                        show_result = None
                        print("return key pressed successfully! :)")
                    if int(user_answer) != int(math_question[2]):
                        print(math_question[2])
                        show_result = True
                if active == True:
        ...
        if show_result == True:
            ...
            if computer_car_rect.y <= endpoint if mph > 0 else computer_car_rect.y <= -50:
                stop_moving2 = True
                COMPUTER_Y = endpoint
                print("comp", COMPUTER_Y, endpoint)
        ...
        if show_result == None:
            ...
            if m == False:
                print(mph, "ghghhgggjhj")
                computer_speed_result = calculate_computer_speed(user_speed=mph)
                print(mph, "fdjgjigjjgdggfgfdgfdgf")
                m = True
            print(computer_speed_result)
            #print("< ", COMPUTER_Y, computer_car_rect.y, computer_speed_result)
